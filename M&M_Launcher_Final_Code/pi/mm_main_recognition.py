...
ap.add_argument("-t ", "--train", type=bool, default=False,
	help="Whether or not to train new data")
...
harrison = ["Harrison1.jpg","Harrison2.jpg","Harrison3.jpg",
            "Harrison4.jpg","Harrison5.jpg","Harrison6.jpg",
            "Harrison7.jpg","Harrison8.jpg","Harrison9.jpg",
            "Harrison10.jpg"]
...
trainDataPath = "/home/pi/Desktop/harryFaces/"
...
if train != True:
    with open('../resources/people_faces.dat', 'rb') as f:
	    people_face_encodings = pickle.load(f)
else:
    for i in range(len(people)):
        for j in range(len(people[i])):
            ...
            #print(j)
            print(".")
            people_images.insert(j, face_recognition.load_image_file(trainDataPath + people[i][j]))
            if train:
                people_face_encodings.insert(j, face_recognition.face_encodings(people_images[j])[0])
...
for i in range(len(people_face_encodings)):
    known_face_encodings.insert(i, people_face_encodings[i])
for i in range(len(people)):
    realFaceCounts.append(0)
    for j in range(len(people[i])):
        known_face_names.insert(j, names[i])

if train:
    with open('../resources/harry_faces.dat', 'wb') as f:
        pickle.dump(people_face_encodings, f)
face_locations = []
face_encodings = []
face_names = []
...
def locateUser():
    ...
    for i in range(0, detections.shape[2]):
        ...
        face_locations.append((ROI_Rect.top(), ROI_Rect.right(), ROI_Rect.bottom(), ROI_Rect.left()))
        ...
        for face_encoding in face_encodings:
            ...
            if True in matches:
                for i in range(len(matches)):
                    if matches[i] == True:
                        face_counts[names.index(known_face_names[i])]+=1
                largest = max(face_counts)
                name = names[face_counts.index(largest)]
